...
# dodać pyvirtualcamera , żeby tworzyć instancje kamery 


# Importy dla PySide6 (Qt)
...
# Importy specyficzne dla Windowsa
...
MIN_LAYER_SIZE = 10 # Minimalny rozmiar warstwy (piksele)
MAX_LAYER_SIZE = 4000 # Maksymalny rozmiar warstwy (piksele)
RESIZE_HANDLE_SIZE = 10 # Rozmiar uchwytu do zmiany rozmiaru w pikselach
MOVE_STEP = 5 # Krok przesuwania warstw strzałkami klawiatury

# Typy źródeł obrazu
SOURCE_TYPE_CAMERA = "Camera"
SOURCE_TYPE_SCREEN_REGION = "Screen Region"

# --- ZMIENNE DLA KONTROLI ZDJĘĆ W Oknie PODGLĄDU (teraz jako Warstwy) ---
class ImageState:
    def __init__(self, name, source_type, initial_width, initial_height, initial_x, initial_y,
        ...
        self.id = str(uuid.uuid4()) # Unikalny identyfikator warstwy
        self.name = name
        self.source_type = source_type # "Camera" lub "Screen Region"
        self.original_image = None
        self.display_width = max(MIN_LAYER_SIZE, initial_width) # Zabezpieczenie przed zerowym rozmiarem
        self.display_height = max(MIN_LAYER_SIZE, initial_height)
        self.x = initial_x
        self.y = initial_y
        self.is_dragging = False
        self.is_on_top = is_on_top # Ta flaga będzie teraz mniej istotna, kolejność będzie z listy
        self.is_visible = is_visible # NOWA FLAGA WIDOCZNOŚCI
        self.drag_offset_x = 0
        self.drag_offset_y = 0
        self.aspect_ratio = initial_width / initial_height if initial_height > 0 else 1.0

        # Specyficzne dla źródła
        self.camera_index = camera_index # Tylko dla SOURCE_TYPE_CAMERA
        self.screen_region = screen_region # {'top': ..., 'left': ..., 'width': ..., 'height': ...}
        self.selected_app_window_info = None # Dodatkowe info o oknie, z którego pochodzi ROI

# --- FUNKCJE POMOCNICZE (NIEZMIENIONE, BO SĄ OGÓLNE) ---

def list_cameras():
    """Wykrywa i wypisuje dostępne kamery internetowe."""
    available_cameras = []
    # Testuj tylko do 5 kamer, żeby nie zawieszać systemu
    for i in range(5):
        # Dodano cv2.CAP_DSHOW dla Windowsa, może pomóc z problemami z kamerą
        # Dodano 100 ms timeout, żeby nie blokować zbyt długo
        cap = cv2.VideoCapture(i, cv2.CAP_DSHOW)
        if cap.isOpened():
            available_cameras.append(i)
            cap.release()
        else:
            # Jeśli nie da się otworzyć, przerwij, żeby nie czekać na nieistniejące kamery
            # Można tu dodać timeout, ale dla prostoty pomijam
            pass
    return available_cameras

# Zmienna globalna do przechowywania informacji o wszystkich oknach
ALL_WINDOWS_INFO = {}

def enum_windows_callback(hwnd, lParam):
    """Callback dla win32gui.EnumWindows - zbiera informacje o oknach."""
    if not win32gui.IsWindowVisible(hwnd):
        return True # Kontynuuj wyliczanie

    if win32gui.IsIconic(hwnd): # Sprawdź, czy okno jest zminimalizowane
        return True # Pomiń zminimalizowane okna
    ...
    # Filtrowanie okien naszego programu, w tym okna selectROI
    # Nazwy okien, które mają być ignorowane
    ignored_window_titles_starts_with = [
        PREVIEW_WINDOW_NAME,
        "Zaznacz obszar do przechwycenia w:",
        "python.exe", # Może to być okno konsoli, którą chcemy zignorować
        "py.exe",
        ...
        "MSCTFIME UI",
        "IME",
        "Pasek zadań" # Dodano Pasek zadań
    ]
    ignored_window_titles_contains = [
        "Windows Defender Notification" # Przykład
    ]
    ...
    # Sprawdź również, czy okno nie jest zbyt małe, aby być sensownym
    if width < 50 or height < 50:
        return True

    global ALL_WINDOWS_INFO # Użyj globalnej zmiennej
    ALL_WINDOWS_INFO[hwnd] = {
        'hwnd': hwnd, # Dodajemy HWND do słownika
        'title': title,
        'rect': rect,
        'pid': process_id,
        'width': width,
...
def get_processes_with_windows_pywin32():
    """
    Pobiera listę unikalnych procesów, które posiadają widoczne okna, używając pywin32.
    Zwraca listę słowników: [{"name": "process_name", "pids": [pid1, pid2], "windows": [...]}]
    """
    if not HAS_WIN32:
        return []

    global ALL_WINDOWS_INFO
    ALL_WINDOWS_INFO = {} # Resetuj zebrane informacje o oknach
    win32gui.EnumWindows(enum_windows_callback, None)

    # Mapowanie PIDów do nazw procesów
    process_pid_to_name = {}
...
class CameraScreenOverlayApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.cap = None # Obiekt VideoCapture (tylko dla kamery)
        self.screen_capture_instance = mss.mss() # Instancja MSS

        # Lista wszystkich aktywnych warstw (obiektów ImageState)
        self.image_states = []
        self.active_camera_layer_id = None # ID aktywnej warstwy kamery (może być tylko jedna)

        # Zmienne dla interakcji myszy
        self.active_draggable_image_state = None # Obiekt ImageState, który jest obecnie przeciągany
        self.active_resizable_image_state = None # Obiekt ImageState, który jest obecnie zmieniany rozmiar
        self.resize_handle_active = None # 'left', 'right', 'top', 'bottom', 'top_left', 'top_right', 'bottom_left', 'bottom_right'
        self.last_mouse_x = -1
        self.last_mouse_y = -1

        self.is_roi_selection_active = False # Flaga aktywności zaznaczania ROI dla nowej warstwy
        self.selected_app_window_info_for_new_layer = None # Przechowuje informacje o wybranym oknie dla nowej warstwy

        self.init_ui() # Inicjalizacja interfejsu użytkownika
        self.init_camera_layer() # Inicjalizacja domyślnej warstwy kamery

        # Timer do odświeżania klatek
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(30) # Odświeżanie co 30 ms (ok. 33 FPS)
    ...
    def init_ui(self):
        ...
        # Combobox do wyboru procesu/aplikacji
        ...
        # Combobox do wyboru okna (podrzędny) - MUSI BYĆ STWORZONY ZANIM populate_app_combobox() zostanie wywołane
        ...
        # Combobox warstw - MUSI BYĆ STWORZONY ZANIM update_layers_combobox() zostanie wywołane
        ...
        # NOWE PRZYCISKI DO ZMIANY KOLEJNOŚCI
        ...
        # Spacer dla panelu warstw
        self.layer_management_layout.addStretch(1)


        # --- Etykieta (QLabel) do wyświetlania obrazu z OpenCV ---
        self.video_label = QLabel(self)
        self.video_label.setAlignment(Qt.AlignCenter)
        self.video_label.setStyleSheet("background-color: black;")
        self.video_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding) # Rozciąga się
        self.main_layout.addWidget(self.video_label)

        # Ustawienie callbacków myszy dla QLabel (aby QLabel reagowało na zdarzenia myszy)
        self.video_label.setMouseTracking(True) # Włącz śledzenie myszy nawet bez przycisku
        ...
        # --- WAŻNE: Wywołaj te funkcje POPULUJĄCE po tym, jak wszystkie QComboBoxy zostały utworzone ---
        self.populate_camera_combobox()
        self.populate_app_combobox()
        self.update_layers_combobox() # To też musi być wywołane po utworzeniu self.layers_combobox

    def init_camera_layer(self):
        """Inicjalizuje domyślną warstwę kamery, ale NIE otwiera jej automatycznie."""
        # Początkowy rozmiar i pozycja kamery to stałe 200x200 i wyśrodkowana
        cam_width = 200
        cam_height = 200
        # Wyśrodkowanie w przewidywanym początkowym rozmiarze okna podglądu
    ...
    def add_layer(self, layer_state):
        """Dodaje nową warstwę do listy i aktualizuje ComboBox warstw."""
        self.image_states.append(layer_state)
        # Sortowanie NIE jest już konieczne tutaj, bo kolejność z listy będzie kolejnością rysowania
        # self.image_states.sort(key=lambda x: (x.source_type != SOURCE_TYPE_CAMERA))

        self.update_layers_combobox()
        # Automatycznie wybierz nowo dodaną warstwę
        index = self.layers_combobox.findData(layer_state.id)
        if index != -1:
            self.layers_combobox.setCurrentIndex(index)

    def remove_layer(self, layer_id):
        """Usuwa warstwę o danym ID i aktualizuje ComboBox warstw."""
        ...
        if layer_to_remove:
            ...
            if layer_id == self.active_camera_layer_id:
                ...
                if self.cap and self.cap.isOpened():
                    self.cap.release()
                    self.cap = None
                # Resetuj combobox wyboru kamery
                self.camera_combobox.setCurrentIndex(
                    self.camera_combobox.findData(None)
                ) # Szukaj indeksu dla userData None (czyli "-- Wybierz kamerę --")
            self.update_layers_combobox()

    def get_layer_by_id(self, layer_id):
        """Zwraca obiekt ImageState dla danego ID warstwy."""
    ...
    def update_layers_combobox(self):
        """Odświeża listę warstw w ComboBoxie."""
        current_selected_id = self.layers_combobox.currentData() # Zachowaj aktualnie wybrany ID
        self.layers_combobox.clear()

        # Kolejność warstw w comboboxie odzwierciedla teraz kolejność rysowania (od dołu do góry)
        # Czyli pierwsza warstwa na liście self.image_states będzie pierwsza w comboboxie
        # i będzie rysowana jako pierwsza (na spodzie).
        # Odwracamy kolejność dla wyświetlania w comboboxie, żeby "na wierzchu" było na górze listy
        # Ale w wewnętrznej liście self.image_states pozostaje od spodu do wierzchu
        for layer_state in reversed(self.image_states): # Odwrócona kolejność do wyświetlenia w comboboxie
        ...
        # Aktywuj lub dezaktywuj przyciski na podstawie liczby warstw
        ...
        # Przywróć poprzedni wybór, jeśli istniał i jest nadal dostępny
        if current_selected_id:
            index = self.layers_combobox.findData(current_selected_id)
            if index != -1:
                self.layers_combobox.setCurrentIndex(index)
            else: # Jeśli wybrana warstwa została usunięta, wybierz pierwszą (teraz ostatnią w odwróconej liście)
                self.layers_combobox.setCurrentIndex(0)
        elif self.layers_combobox.count() > 0: # Jeśli nie było wyboru, a są warstwy, wybierz pierwszą
            self.layers_combobox.setCurrentIndex(0)

        # Po aktualizacji, upewnij się, że combobox warstw odzwierciedla stan przycisków aktywacji/deaktywacji
        self.on_layer_selection_changed(self.layers_combobox.currentIndex())


    def on_layer_selection_changed(self, index):
        """Obsługuje zmianę wyboru w ComboBoxie warstw i aktualizuje stan przycisków."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            selected_layer_index_in_list = -1
            # Musimy znaleźć indeks w self.image_states, a nie w comboboxie, bo jest odwrócony
            ...
            if selected_layer_index_in_list != -1:
                # Włącz/wyłącz przyciski "Przesuń w górę/dół"
                # "Przesuń w górę" oznacza "bliżej końca listy" (czyli do index-1 w comboboxie)
                # "Przesuń w dół" oznacza "bliżej początku listy" (czyli do index+1 w comboboxie)
                
                # "Na wierzch" jest, jeśli nie jest ostatni na liście
                self.move_to_front_button.setEnabled(selected_layer_index_in_list < len(self.image_states) - 1)
                # "Na spód" jest, jeśli nie jest pierwszy na liście
                self.move_to_back_button.setEnabled(selected_layer_index_in_list > 0)
                
                # "Przesuń w górę" oznacza przesunięcie w liście self.image_states do wyższego indeksu (czyli dalej od początku)
                self.move_up_button.setEnabled(selected_layer_index_in_list < len(self.image_states) - 1)
                # "Przesuń w dół" oznacza przesunięcie w liście self.image_states do niższego indeksu (czyli bliżej początku)
    ...
    def toggle_selected_layer_visibility(self):
        """Przełącza widoczność wybranej warstwy."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            ...
            if layer:
                ...
                self.update_layers_combobox() # Odśwież, aby zmienić ✅/❌
                # Przywróć wybór w comboboxie
                index = self.layers_combobox.findData(layer.id)
                if index != -1:
                    self.layers_combobox.setCurrentIndex(index)

    def move_selected_layer_up(self):
        """Przesuwa wybraną warstwę o jedną pozycję w górę (bliżej wierzchu)."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            layer_index = -1
    ...
    def move_selected_layer_down(self):
        """Przesuwa wybraną warstwę o jedną pozycję w dół (bliżej spodu)."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            layer_index = -1
            for i, layer_state in enumerate(self.image_states):
    ...
    def move_selected_layer_to_front(self):
        """Przenosi wybraną warstwę na wierzch (koniec listy)."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            layer = self.get_layer_by_id(selected_layer_id)
            if layer and layer in self.image_states:
                # Sprawdź, czy warstwa jest już na wierzchu (ostatnia w liście)
                if not self.image_states or self.image_states[-1].id == layer.id:
                    return

                # Usuń i dodaj na koniec
    ...
    def move_selected_layer_to_back(self):
        """Przenosi wybraną warstwę na spód (początek listy)."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            layer = self.get_layer_by_id(selected_layer_id)
            if layer and layer in self.image_states:
                # Sprawdź, czy warstwa jest już na spodzie (pierwsza w liście)
                if not self.image_states or self.image_states[0].id == layer.id:
                    return

                self.image_states.remove(layer)
                self.image_states.insert(0, layer) # Wstaw na początek
                self.update_layers_combobox()
                # Przywróć wybór w comboboxie
                index = self.layers_combobox.findData(layer.id)
                if index != -1:
    ...
    def populate_camera_combobox(self):
        """Wypełnia combobox listą dostępnych kamer."""
        self.camera_combobox.clear()
        self.available_cameras = list_cameras()

        # Dodaj domyślną opcję, która nie wybiera żadnej kamery
        ...
        for i in self.available_cameras:
            self.camera_combobox.addItem(f"Kamera {i}", userData=i) # userData przechowuje indeks kamery
        self.camera_combobox.setEnabled(True)
        # Pozostaw wybrane "-- Wybierz kamerę --"
        self.camera_combobox.setCurrentIndex(0)


    def select_camera_source(self, index):
        """
        Otwiera wybraną kamerę i przypisuje ją do aktywnej warstwy kamery.
        Jeśli warstwa kamery nie istnieje, tworzy ją.
        """
        camera_index = self.camera_combobox.itemData(index)

        camera_layer = self.get_layer_by_id(self.active_camera_layer_id)

        # Jeśli nie ma aktywnej warstwy kamery, a wybrano kamerę, utwórz ją
        if camera_index is not None and camera_layer is None:
            # Tworzenie nowej warstwy kamery
            cam_width = 200
            cam_height = 200
        ...
        # Jeśli aktualnie jest otwarta jakaś kamera, zwolnij ją
        if self.cap and self.cap.isOpened():
            self.cap.release()
            self.cap = None
            print("Poprzednia kamera zwolniona.")

        if camera_layer: # Upewnij się, że warstwa kamery istnieje
            if camera_index is None: # Wybrano "-- Wybierz kamerę --"
                camera_layer.original_image = None
                camera_layer.camera_index = None # Usuń indeks kamery
            ...
            # Próbujemy ustawić domyślne rozmiary, ale nie są one gwarantowane
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, DEFAULT_CAM_WIDTH)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, DEFAULT_CAM_HEIGHT)

            # Odczytaj pierwszą klatkę, aby zainicjować kamerę i pobrać jej rzeczywiste wymiary
            ...
            if ret:
                ...
                camera_layer.camera_index = camera_index # Zapisz indeks kamery w obiekcie warstwy
                # Przypisz pierwszą klatkę, aby obraz pojawił się od razu
                camera_layer.original_image = cv2.flip(frame_camera, 1) # Lustrzane odbicie

                # Dostosuj display_height warstwy do jej display_width, zachowując proporcje
                # Upewnij się, że rozmiar nie jest zerowy
                ...
                else: # Jeśli aspectRatio jest 0 (np. przez błąd dzielenia), ustaw na kwadrat
    ...
    def populate_app_combobox(self):
        """Wypełnia combobox listą dostępnych aplikacji/procesów."""
        self.app_combobox.clear()
        self.window_combobox.clear()
        self.window_combobox.setEnabled(False)
        self.add_roi_layer_button.setEnabled(False) # Wyłącz przycisk "Dodaj warstwę z ROI"
        ...
        self.app_combobox.addItem("-- Wybierz aplikację --", userData=None) # Domyślna pusta opcja
    ...
    def select_application_for_roi(self, index):
        """Obsługuje wybór aplikacji z comboboxa i wypełnia combobox okien."""
        self.window_combobox.clear()
        self.window_combobox.setEnabled(False)
        self.add_roi_layer_button.setEnabled(False) # Wyłącz, dopóki nie będzie wybrane okno
        ...
        if selected_proc_info and selected_proc_info.get('windows'):
            ...
            if len(windows_list) > 1:
                ...
                self.selected_app_window_info_for_new_layer = None # Resetuj, ponieważ trzeba wybrać konkretne okno
            else:
                # Jeśli tylko jedno okno, wybierz je automatycznie
                ...
                self.window_combobox.setEnabled(False) # Wyłącz, skoro automatycznie wybrano
                self.add_roi_layer_button.setEnabled(True) # Włącz przycisk "Dodaj warstwę z ROI"
    ...
    def select_window_for_roi(self, index):
        """Obsługuje wybór konkretnego okna z comboboxa okien dla nowej warstwy ROI."""
    ...
    def start_roi_selection_for_new_layer(self):
        """Rozpoczyna proces zaznaczania ROI dla nowej warstwy z użyciem cv2.selectROI."""
        ...
        self.hide() # Ukryj główne okno GUI na czas zaznaczania ROI

        # Użyj funkcji z poprzedniego kodu do zaznaczania ROI
        roi_result = self._draw_roi_on_selected_window(self.selected_app_window_info_for_new_layer)

        self.show() # Pokaż główne okno GUI ponownie po zakończeniu zaznaczania
        ...
        if roi_result:
            ...
            # Obliczanie początkowego rozmiaru nowej warstwy na podstawie proporcji ROI i połowy okna podglądu
            ...
            new_layer.selected_app_window_info = window_info # Zapisz info o oknie
    ...
    def _draw_roi_on_selected_window(self, app_window_info):
        """
        Pozwala użytkownikowi interaktywnie zaznaczyć obszar w wybranym oknie aplikacji.
        Zwraca tuple (selected_screen_region, app_window_info) lub None.
        """
        ...
        try:
            ...
            if sct_img is None or not sct_img.pixels: # Sprawdź czy zrzut nie jest pusty
        ...
        if HAS_WIN32:
            roi_hwnd = None
            # Poczekaj chwilę, aż okno OpenCV zostanie faktycznie stworzone
            for _ in range(20): # Sprawdź 20 razy z małym opóźnieniem
            ...
            if roi_hwnd:
                try:
                    win32gui.ShowWindow(roi_hwnd, win32con.SW_RESTORE) # Przywróć okno jeśli zminimalizowane
                    win32gui.SetForegroundWindow(roi_hwnd) # Ustaw na pierwszy plan
        ...
        if w > 0 and h > 0:
            ...
            # W OpenCV ROI może zwrócić (0,0,0,0) gdy anulowane
            if w == 0 or h == 0:
    ...
    def update_frame(self):
        """Pobiera klatki i aktualizuje wyświetlanie dla wszystkich widocznych warstw."""
        ...
        if current_preview_window_width <= 0 or current_preview_window_height <= 0:
            # Okno jest zminimalizowane lub ma zerowy rozmiar, nie rysuj
        ...
        # Pobieranie obrazów dla wszystkich warstw
        for layer_state in self.image_states:
            if not layer_state.is_visible:
                # Jeśli warstwa niewidoczna, ustaw original_image na None, żeby nie była rysowana
                layer_state.original_image = None
                continue

            if layer_state.source_type == SOURCE_TYPE_CAMERA:
                # Upewnij się, że self.cap jest aktywne i to jest faktycznie nasza warstwa kamery
                if self.cap and self.cap.isOpened() and layer_state.id == self.active_camera_layer_id:
                    ret, frame_camera = self.cap.read()
                    if ret:
                        frame_camera = cv2.flip(frame_camera, 1) # Lustrzane odbicie
                    ...
                    layer_state.original_image = None # Upewnij się, że jest None, jeśli kamera nieaktywna
                ...
                if layer_state.screen_region:
                    try:
                        ...
                        layer_state.original_image = None
                        # print(f"Błąd zrzutu ekranu dla warstwy '{layer_state.name}'. Obszar mógł zniknąć lub jest niedostępny.")
                    except Exception as e:
                        layer_state.original_image = None
                        # print(f"Nieoczekiwany błąd podczas zrzutu ekranu dla warstwy '{layer_state.name}': {e}")
                else:
                    layer_state.original_image = None

        # Rysowanie warstw w kolejności (od spodu do wierzchu)
        for layer_state in self.image_states:
            # Pomiń warstwy niewidoczne, bez obrazu, lub o zerowym rozmiarze
            ...
            # Korygowanie współrzędnych, aby obraz nie wyszedł poza okno podglądu
            paste_x1 = max(0, x1)
            paste_y1 = max(0, y1)
            paste_x2 = min(current_preview_window_width, x2)
            paste_y2 = min(current_preview_window_height, y2)

            # Wylicz, który fragment scaled_image ma być wklejony
            src_x1 = max(0, -x1)
            src_y1 = max(0, -y1)
        ...
        self.update_image_signal.emit(display_frame) # Emituj sygnał z gotową klatką

    def update_video_label(self, cv_img):
        """Aktualizuje Qlabel obrazem z OpenCV."""
        if cv_img is None:
            # Wyświetl czarny obraz, jeśli cv_img jest None
            height, width = self.video_label.height(), self.video_label.width()
            if height <= 0 or width <= 0: # Upewnij się, że są poprawne wymiary
                height = INITIAL_PREVIEW_WINDOW_HEIGHT
                width = INITIAL_PREVIEW_WINDOW_WIDTH
            cv_img = np.zeros((height, width, 3), dtype=np.uint8)

        # Konwersja obrazu OpenCV do QImage
        if len(cv_img.shape) == 3: # Kolorowy
        ...
        # Skalowanie QImage do rozmiaru QLabel, utrzymując proporcje
        pixmap = QPixmap.fromImage(q_img)
        self.video_label.setPixmap(pixmap.scaled(self.video_label.size(),
                                                 Qt.KeepAspectRatio,
                                                 Qt.SmoothTransformation))

    # --- Mouse event handlers for QLabel ---
    def get_resize_handle_type(self, img_state, mouse_x, mouse_y):
        """
        Sprawdza, czy kursor myszy znajduje się nad uchwytem do zmiany rozmiaru.
        Zwraca typ uchwytu ('top_left', 'top', 'right', itp.) lub None.
        """
    ...
    def set_cursor_for_resize_handle(self, handle_type):
        """Ustawia odpowiedni kursor dla danego typu uchwytu."""
        if handle_type == 'top_left' or handle_type == 'bottom_right':
            ...
            self.video_label.setCursor(Qt.ArrowCursor) # Domyślny kursor

    def preview_mouse_press_event(self, event: QMouseEvent):
        # Pobierz pozycję myszy względem QLabel (nie globalnie)
        x, y = event.position().x(), event.position().y()
        self.last_mouse_x = x
        self.last_mouse_y = y

        self.active_draggable_image_state = None # Reset aktywnego stanu przeciągania
        self.active_resizable_image_state = None # Reset aktywnego stanu zmiany rozmiaru
        self.resize_handle_active = None

        # Sprawdź warstwy od wierzchu do spodu (czyli od końca listy do początku)
        # Dzięki temu kliknięcie w nakładające się obrazy wybierze ten na wierzchu
        for img_state in reversed(self.image_states):
            ...
            if handle_type:
                # Jeśli kliknięto uchwyt do zmiany rozmiaru
                ...
                return # Obsłużono zdarzenie (zmiana rozmiaru)

            # Sprawdź, czy mysz jest w obrębie obrazu (do przeciągania)
            if img_state.x <= x <= img_state.x + img_state.display_width and \
               img_state.y <= y <= img_state.y + img_state.display_height:
                ...
                self.video_label.setCursor(Qt.ClosedHandCursor) # Kursor do przeciągania
                return # Obsłużono zdarzenie (przeciąganie)
    ...
    def preview_mouse_release_event(self, event: QMouseEvent):
        ...
        self.resize_handle_active = None
        self.video_label.setCursor(Qt.ArrowCursor) # Przywróć domyślny kursor
        self.last_mouse_x = -1
        self.last_mouse_y = -1
    ...
    def preview_mouse_move_event(self, event: QMouseEvent):
        ...
        if self.active_resizable_image_state and self.resize_handle_active:
            ...
            # Oblicz nowe wymiary i pozycję w zależności od aktywnego uchwytu
            ...
            # Ponowne obliczenie pozycji, jeśli rozmiar został ograniczony
            if 'left' in self.resize_handle_active:
        ...
        # Jeśli ani nie przeciągamy, ani nie zmieniamy rozmiaru, zmień kursor na podstawie uchwytów
        else:
            found_handle = False
            for img_state in reversed(self.image_states):
    ...
    def preview_mouse_wheel_event(self, event: QWheelEvent):
        # Użyj event.position() dla lokalnych współrzędnych QLabel
        x, y = event.position().x(), event.position().y()
        delta = event.angleDelta().y() / 120 # Standardowe przesunięcie kółka myszy
        zoom_factor = 1.0 + delta * 0.1 # Zwiększono szybkość zoomu do 10%

        target_state = None
        # Znajdź warstwę pod kursorem (od wierzchu)
        for img_state in reversed(self.image_states):
        ...
        if target_state is None: # Jeśli kursor nie jest nad żadnym obrazem, nie rób nic
            return

        # Jeśli warstwa ma obraz źródłowy (żeby mieć co skalować)
        if target_state.original_image is not None:
            ...
            # Zastosuj ograniczenia rozmiaru
            new_width = max(MIN_LAYER_SIZE, min(MAX_LAYER_SIZE, new_width))
            if target_state.aspect_ratio > 0:
                new_height = int(new_width / target_state.aspect_ratio)
            new_height = max(MIN_LAYER_SIZE, min(MAX_LAYER_SIZE, new_height))

            # Upewnij się, że proporcje są zachowane również dla wysokości (jeśli nastąpiło zaokrąglenie)
            if target_state.aspect_ratio > 0:
            ...
            # Aby zoom był "do kursora", przesuwamy obraz
            # Oblicz relatywną pozycję kursora w obrazie przed skalowaniem
            ...
            # Oblicz nową pozycję tak, aby punkt pod kursorem pozostał pod kursorem
            target_state.x = int(x - rel_x * target_state.display_width)
            target_state.y = int(y - rel_y * target_state.display_height)

    def keyPressEvent(self, event):
        """Obsługuje zdarzenia klawiatury dla przesuwania warstw."""
        selected_layer_id = self.layers_combobox.currentData()
        if selected_layer_id:
            layer = self.get_layer_by_id(selected_layer_id)
            if layer:
                if event.key() == Qt.Key_Left:
                    ...
                    layer.y += MOVE_STEP
                # Możesz dodać inne klawisze dla skalowania, rotacji itp.
                # print(f"Warstwa '{layer.name}' przesunięta do: ({layer.x}, {layer.y})")
                # Nie trzeba wywoływać update_frame() ręcznie, timer to zrobi.
                event.accept() # Oznacz zdarzenie jako obsłużone
            else:
                event.ignore() # Przekaż zdarzenie dalej
        else:
            event.ignore() # Przekaż zdarzenie dalej

    def closeEvent(self, event):
        """Zwalnia zasoby po zamknięciu okna."""
        if self.cap and self.cap.isOpened():
            self.cap.release()
        self.timer.stop()
